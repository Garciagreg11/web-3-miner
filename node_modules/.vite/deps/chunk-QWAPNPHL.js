...
function verify2(options) {
  ...
  return secp256r12.verify(signature, payload instanceof Uint8Array ? payload : fromHex(payload), toHex(publicKey).substring(2), { lowS: true, ...hash5 ? { prehash: true } : {} });
}
...
function toPublicKey(coseKey) {
  const decoded = decode2(coseKey);
  const x = decoded["-2"];
  const y = decoded["-3"];
  if (!(x instanceof Uint8Array) || !(y instanceof Uint8Array))
    throw new InvalidCoseKeyError();
  return from3(new Uint8Array([4, ...x, ...y]));
}
...
function getAuthenticatorData(options = {}) {
  ...
  const credIdLen = fromNumber2(credential.id.length, { size: 2 });
  const coseKey = fromPublicKey2(credential.publicKey);
  return concat2(base, aaguid, credIdLen, credentialId, coseKey);
}
function getClientDataJSON(options) {
  const { challenge, crossOrigin = false, extraClientData, origin = window.location.origin, type: type2 = "webauthn.get" } = options;
...
var InvalidConfigError = class extends BaseError {
  constructor({ reason }) {
...
function deserialize(value) {
  ...
  if (typeId === serializedWebAuthnType) {
    ...
    for (let split = 37; split < webauthnDataSize; split++) {
      const potentialJson = toString2(slice2(webauthnData, split));
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "p256") {
    const p2563 = envelope;
    return concat2(serializedP256Type, fromNumber2(p2563.signature.r, { size: 32 }), fromNumber2(p2563.signature.s, { size: 32 }), fromNumber2(p2563.publicKey.x, { size: 32 }), fromNumber2(p2563.publicKey.y, { size: 32 }), fromNumber2(p2563.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "webAuthn") {
    const webauthn = envelope;
...
function sortMultisigApprovals(value) {
  ...
  const digest = getSignPayload3("genesisConfig" in value && value.genesisConfig ? { payload, genesisConfig: value.genesisConfig } : {
    payload,
...
function toRpc2(envelope) {
  ...
  if (type2 === "p256") {
    ...
    return {
      ...
      pubKeyX: fromNumber2(p2563.publicKey.x, { size: 32 }),
      pubKeyY: fromNumber2(p2563.publicKey.y, { size: 32 }),
      r: fromNumber2(p2563.signature.r, { size: 32 }),
      s: fromNumber2(p2563.signature.s, { size: 32 }),
      type: "p256"
    };
  }
  if (type2 === "webAuthn") {
...
function getSignPayload5(authorization) {
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload6(from18(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload6(from18(envelope)),
...
function getSignPayload6(envelope, options = {}) {
...
var InvalidMagicError = class extends BaseError {
  constructor({ address: address2 }) {
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-DPB5NFSZ.js");
  ...
  return {
    spawn(index, onMessage, onError) {
      ...
      worker.on("exit", (code) => {
        if (code !== 0)
          onError(new BaseError(`A salt mining worker exited with code "${code}".`));
      });
      return {
        postMessage(data) {
...
function mineSaltWithWorkerPool(value) {
  ...
  return new Promise((resolve3, reject) => {
    ...
    const fail = (error) => {
      ...
      (_a3 = value.signal) == null ? void 0 : _a3.removeEventListener("abort", onAbort);
      terminateAll();
      reject(error instanceof Error ? error : new BaseError("Failed to mine virtual master salt."));
    };
    const onMessage = (msg) => {
      if (settled)
        return;
      switch (msg.type) {
        ...
        case "done":
          ...
          return;
        case "error":
          fail(new BaseError(msg.message));
          return;
      }
    };
...
function assertWorkers(workers) {
  if (Number.isSafeInteger(workers) && workers >= 0)
    return;
  throw new BaseError(`Workers "${workers}" is invalid. Expected a non-negative safe integer.`);
}
function getAbortError(signal) {
  const reason = signal == null ? void 0 : signal.reason;
  if (reason instanceof Error)
    return reason;
  return new BaseError("The operation was aborted.");
}
...
function assertCount(count) {
  if (Number.isSafeInteger(count) && count > 0)
    return;
  throw new BaseError(`Count "${count}" is invalid. Expected a positive safe integer.`);
}
...
function assertValidMasterAddress(address2) {
  ...
  if (normalized === zeroAddress2)
    throw new BaseError("Virtual master address cannot be the zero address.");
  if (isVirtual(address2))
    throw new BaseError("Virtual master address cannot itself be a virtual address.");
  if (normalized.startsWith(tip20Prefix2))
    throw new BaseError("Virtual master address cannot be a TIP-20 token address.");
}
...
function getSignPayload7(authentication, options = {}) {
...
var InvalidSerializedError3 = class extends BaseError {
  constructor({ reason, serialized }) {
...
var MissingSignatureError = class extends BaseError {
  constructor() {
...
__export(accessKey_exports, {
  ...
  burnWitnessSync: () => burnWitnessSync,
  ...
  watchAdminAuthorized: () => watchAdminAuthorized,
  watchWitness: () => watchWitness,
  watchWitnessBurned: () => watchWitnessBurned
});
...
__export(Abis_exports, {
  ...
  nonce: () => nonce,
  receivePolicyGuard: () => receivePolicyGuard,
  signatureVerifier: () => signatureVerifier,
  stablecoinDex: () => stablecoinDex,
  tip20: () => tip20,
  tip20ChannelReserve: () => tip20ChannelReserve,
  tip20Factory: () => tip20Factory,
  tip403Registry: () => tip403Registry,
  validatorConfig: () => validatorConfig,
  validatorConfigV2: () => validatorConfigV2
});
...
async function getSharedSecret2(options) {
  const { as = "Hex", privateKey, publicKey } = options;
...
async function verify6(options) {
  ...
  if (lowS && signature.s > p2562.CURVE.n / 2n)
    return false;
...
function deserialize5(serializedTransaction) {
  const type2 = slice2(serializedTransaction, 0, 1);
...
function deserializeTempo(serializedTransaction) {
  const { feePayerSignature, nonce: nonce2, ...tx } = TxEnvelopeTempo_exports.deserialize(serializedTransaction);
  return {
    ...tx,
    nonce: Number(nonce2 ?? 0n),
...
// node_modules/viem/_esm/tempo/Account.js
function from21(parameters2) {
  const { access } = parameters2;
  if (access)
    return fromAccessKey(parameters2);
  return fromRoot(parameters2);
}
...
function fromSecp256k1(privateKey, options = {}) {
  ...
  return from21({
    ...
    async sign(parameters2) {
      const { hash: hash5 } = parameters2;
      const signature = sign({ payload: hash5, privateKey });
      return toHex3(signature);
    }
  });
}
function fromMultisig(config) {
  const normalized = MultisigConfig_exports.from(config);
  const address2 = checksum(MultisigConfig_exports.getAddress(normalized));
  return {
    address: address2,
    config: normalized,
    publicKey: "0x",
    ...
    async signTransaction(transaction, options) {
      const { serializer = serialize6 } = options ?? {};
...
function fromWebAuthnP256(credential, options = {}) {
  ...
  return from21({
    ...
    publicKey,
    async sign({ hash: hash5 }) {
      const { metadata, signature } = await sign4({
        ...options,
        challenge: hash5,
        credentialId: id
      });
...
function fromWebCryptoP256(keyPair, options = {}) {
  ...
  return from21({
    ...
    publicKey,
    async sign({ hash: hash5 }) {
      const signature = await sign5({ payload: hash5, privateKey });
      return SignatureEnvelope_exports.serialize({
        signature,
        prehash: true,
        publicKey,
...
async function signVoucher(account, parameters2) {
  const hash5 = getVoucherSignPayload2(parameters2);
  if (isAccessKeyAccount(account))
    return account.sign({ hash: hash5, raw: true });
  return account.sign({ hash: hash5 });
}
function getVoucherSignPayload2(parameters2) {
  const { chainId, channel, cumulativeAmount } = parameters2;
  const channelId = typeof channel === "string" ? channel : Channel_exports.computeId(channel, {
    chainId
  });
  return Channel_exports.getVoucherSignPayload({
    chainId,
    channelId,
    cumulativeAmount
  });
...
function fromBase(parameters2) {
  const { keyType = "secp256k1", parentAddress, source = "privateKey", internal_version = "v2" } = parameters2;
  const address2 = parentAddress ?? fromPublicKey(parameters2.publicKey);
  const publicKey = toHex(parameters2.publicKey, {
    includePrefix: false
  });
  async function sign6({ hash: hash5, raw }) {
    if (raw)
      return await parameters2.sign({ hash: hash5 });
    const innerHash = parentAddress && internal_version === "v2" ? keccak256(concat2("0x04", hash5, parentAddress)) : hash5;
    const signature = await parameters2.sign({ hash: innerHash });
    if (parentAddress)
  ...
  return {
    address: checksum(address2),
    keyType,
    sign: sign6,
    async signAuthorization(parameters3) {
      const { chainId, nonce: nonce2 } = parameters3;
      const address3 = parameters3.contractAddress ?? parameters3.address;
      const signature = await sign6({
        hash: hashAuthorization({ address: address3, chainId, nonce: nonce2 })
      });
    ...
    async signMessage(parameters3) {
      const { message } = parameters3;
      return await sign6({ hash: hashMessage(message) });
    },
    async signTransaction(transaction, options) {
      const { serializer = serialize6 } = options ?? {};
      const presign = (() => {
      ...
      if (multisig) {
        const digest = MultisigConfig_exports.getSignPayload({
          payload,
          genesisConfig: multisig
        });
        return await sign6({ hash: digest, raw: true });
      }
      const signature = await sign6({ hash: payload });
      const envelope = SignatureEnvelope_exports.from(signature);
      return await serializer(transaction, envelope);
    },
    async signTypedData(typedData) {
      return await sign6({ hash: hashTypedData(typedData) });
    },
    async signVoucher(parameters3) {
      const hash5 = getVoucherSignPayload2(parameters3);
      if (parentAddress)
        return await sign6({ hash: hash5, raw: true });
      return await sign6({ hash: hash5 });
    },
    publicKey,
    source,
    type: "local"
...
function fromRoot(parameters2) {
  const account = fromBase(parameters2);
  return {
    ...account,
    source: "root",
    async signKeyAuthorization(key, parameters3) {
      const { chainId, expiry, limits, scopes, witness, admin } = parameters3;
      const { accessKeyAddress, keyType: type2 } = resolveAccessKey(key);
      const restrictions = admin ? {} : { expiry, limits, scopes };
...
function fromAccessKey(parameters2) {
  const { access, keyAuthorizationManager } = parameters2;
  const { address: parentAddress } = parseAccount(access);
  const account = fromBase({ ...parameters2, parentAddress });
  return {
    ...account,
    accessKeyAddress: fromPublicKey(parameters2.publicKey),
...
__export(Addresses_exports, {
  ...
  accountKeychain: () => accountKeychain2,
  accountRegistrar: () => accountRegistrar,
  addressRegistry: () => addressRegistry2,
  feeManager: () => feeManager2,
  nonceManager: () => nonceManager,
  pathUsd: () => pathUsd,
  receivePolicyGuard: () => receivePolicyGuard2,
  signatureVerifier: () => signatureVerifier2,
  stablecoinDex: () => stablecoinDex2,
  tip20Factory: () => tip20Factory2,
  tip403Registry: () => tip403Registry2,
  validator: () => validator,
  zoneOutbox: () => zoneOutbox
});
...
var addressRegistry2 = "0xfdc0000000000000000000000000000000000000";
...
var receivePolicyGuard2 = "0xB10C000000000000000000000000000000000000";
...
var validator = "0xcccccccc00000000000000000000000000000000";
var zoneOutbox = "0x1c00000000000000000000000000000000000002";

// node_modules/viem/_esm/tempo/Hardfork.js
var Hardfork_exports = {};
__export(Hardfork_exports, {
  hardforks: () => hardforks,
  lt: () => lt
});
var hardforks = [
...
async function authorizeSync(client, parameters2) {
  const { throwOnReceiptRevert = true, ...rest } = parameters2;
...
async function burnWitnessSync(client, parameters2) {
  const { throwOnReceiptRevert = true, ...rest } = parameters2;
...
async function revokeSync(client, parameters2) {
  const { throwOnReceiptRevert = true, ...rest } = parameters2;
...
async function signAuthorization(client, parameters2) {
  var _a2;
  const { accessKey, chainId = (_a2 = client.chain) == null ? void 0 : _a2.id, ...rest } = parameters2;
  const account_ = rest.account ?? client.account;
  if (!account_)
    throw new Error("account is required.");
  if (!chainId)
...
async function updateLimitSync(client, parameters2) {
  const { throwOnReceiptRevert = true, ...rest } = parameters2;
...
function watchAdminAuthorized(client, parameters2) {
  const { onAuthorized, ...rest } = parameters2;
...
function watchWitness(client, parameters2) {
  const { onWitness, ...rest } = parameters2;
  return watchContractEvent(client, {
    ...rest,
    address: accountKeychain2,
    abi: accountKeychain,
...
function watchWitnessBurned(client, parameters2) {
  const { onBurned, ...rest } = parameters2;
  return watchContractEvent(client, {
    ...rest,
    address: accountKeychain2,
    abi: accountKeychain,
...
var chainConfig = {
  ...
  async verifyHash(client, parameters2) {
    const { address: address2, hash: hash5, signature, mode } = parameters2;
    const envelope = (() => {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode === "allowAccessKey") {
...
export {
  ...
  toRpc5 as toRpc,
  Transaction_exports,
  fromRpc7 as fromRpc2,
  toRpc7 as toRpc2,
  Log_exports,
  fromRpc8 as fromRpc3,
  toRpc8 as toRpc3,
  ...
  validatorConfig,
  Abis_exports,
  createKeyPair2 as createKeyPair,
  WebCryptoP256_exports,
  deserialize5 as deserialize,
  Transaction_exports3 as Transaction_exports2,
  signVoucher,
  Account_exports,
  addressRegistry2,
  feeManager2,
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-QWAPNPHL.js.map
